...
########################################################################################       
# Running the BUG Model                                                              
########################################################################################      
results_bivariatebayesmodel_RE <- run.jags( model="MP1_WithRE_V4.bug", 
                                            ...
                                            burnin = 15000, sample = 10000, adapt = 10000, thin=10);
########################################################################################       
# Showing the results from the BUG Model                                                              
########################################################################################  
results_bivariatebayesmodel_RE

pdf( file='BayesianHierarchicalModel_DensityMixing.pdf')
par( mai=c(.8,.5,.5,.1)) 
plot( results_bivariatebayesmodel_RE, layout=c(3,4))
dev.off()

# Save the summary of the MCMC simulation in a txt-file
sink("BayesianHierarchicalModel_ParametersResults.txt")
